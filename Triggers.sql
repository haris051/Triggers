...
CREATE TRIGGER `TR_PAYMENT_SENT_UPT` AFTER UPDATE ON `payment_sent` FOR EACH ROW BEGIN
	...
	if (OLD.ID <> NEW.ID OR OLD.AMOUNT <> NEW.AMOUNT OR OLD.CASH_ACC_ID <> NEW.CASH_ACC_ID OR OLD.PS_ENTRY_DATE <> NEW.PS_ENTRY_DATE OR OLD.PAYPAL_TRANSACTION_ID <> NEW.PAYPAL_TRANSACTION_ID OR OLD.COMPANY_ID <> NEW.COMPANY_ID)
		then
...
CREATE TRIGGER `TR_RECEIVE_MONEY_UPT` AFTER UPDATE ON `receive_money` FOR EACH ROW BEGIN
	...
	if (OLD.ID <> NEW.ID OR OLD.Amount <> NEW.Amount OR OLD.CASH_ACC_ID <> NEW.CASH_ACC_ID OR OLD.GL_ACC_ID <> NEW.GL_ACC_ID OR OLD.RM_ENTRY_DATE <> NEW.RM_ENTRY_DATE OR OLD.PAYPAL_TRANSACTION_ID <> NEW.PAYPAL_TRANSACTION_ID OR OLD.COMPANY_ID <> NEW.COMPANY_ID)
	   then 
...
CREATE TRIGGER `TR_PAYMENTS_UPT` AFTER UPDATE ON `payments` FOR EACH ROW BEGIN
	...
	IF (OLD.ID <> NEW.ID OR OLD.REMAINING_AMOUNT <> NEW.REMAINING_AMOUNT OR OLD.CASH_ACC_ID <> NEW.CASH_ACC_ID OR OLD.PAY_ENTRY_DATE <> NEW.PAY_ENTRY_DATE OR OLD.PAY_REFERENCE <> NEW.PAY_REFERENCE OR OLD.COMPANY_ID <> NEW.COMPANY_ID)
	   then 
...
CREATE TRIGGER `TR_RECEIPTS_UPT` AFTER UPDATE ON `receipts` FOR EACH ROW BEGIN
  ...
  IF (OLD.ID <> NEW.ID OR OLD.REMAINING_AMOUNT <> NEW.REMAINING_AMOUNT OR OLD.CASH_ACC_ID <> NEW.CASH_ACC_ID OR OLD.R_ENTRY_DATE <> NEW.R_ENTRY_DATE OR OLD.REC_REFERENCE <> NEW.REC_REFERENCE OR OLD.COMPANY_ID <> NEW.COMPANY_ID)
    then 
...
CREATE TRIGGER `TR_CHARGES_UPT` AFTER UPDATE ON `charges` FOR EACH ROW BEGIN
	...
	IF (OLD.ID <> NEW.ID OR OLD.REMAINING_AMOUNT <> NEW.REMAINING_AMOUNT OR OLD.CASH_ACC_ID <> NEW.CASH_ACC_ID OR OLD.C_ENTRY_DATE <> NEW.C_ENTRY_DATE OR OLD.C_REFERENCE <> NEW.C_REFERENCE OR OLD.COMPANY_ID <> NEW.COMPANY_ID)
		then 
...
CREATE TRIGGER `TR_PARTIAL_CREDIT_UPT` AFTER UPDATE ON `partial_credit` FOR EACH ROW BEGIN
	...
	if (OLD.ID <> NEW.ID OR OLD.TOTAL_AMOUNT <> NEW.TOTAL_AMOUNT OR OLD.CASH_ACC_ID <> NEW.CASH_ACC_ID OR OLD.PC_ENTRY_DATE <> NEW.PC_ENTRY_DATE OR OLD.PAYPAL_TRANSACTION_ID <> NEW.PAYPAL_TRANSACTION_ID OR OLD.COMPANY_ID <> NEW.COMPANY_ID)
	   then 
...
CREATE TRIGGER `TR_RECEIVE_ORDER_UPT` AFTER UPDATE ON `receive_order` FOR EACH ROW BEGIN
	...
	if(OLD.ID <> NEW.ID OR OLD.RECEIVE_TOTAL_AMOUNT <> NEW.RECEIVE_TOTAL_AMOUNT OR OLD.AP_ACC_ID <> NEW.AP_ACC_ID OR OLD.RECEIVE_ENTRY_DATE <> NEW.RECEIVE_ENTRY_DATE OR OLD.PAYPAL_TRANSACTION_ID <> NEW.PAYPAL_TRANSACTION_ID OR OLD.COMPANY_ID <> NEW.COMPANY_ID)
		then 
...
CREATE TRIGGER `TR_RECEIVE_ORDER_DETAIL_UPT` AFTER UPDATE ON `receive_order_detail` FOR EACH ROW BEGIN
	...
	IF (OLD.RECEIVE_ORDER_ID <> NEW.RECEIVE_ORDER_ID OR OLD.ID <> NEW.ID OR OLD.AMOUNT <> NEW.AMOUNT OR OLD.GL_ACC_ID <> NEW.GL_ACC_ID OR OLD.ENTRY_DATE <> NEW.ENTRY_DATE)
		then 
...
CREATE TRIGGER `TR_VENDOR_CREDIT_MEMO_UPT` AFTER UPDATE ON `vendor_credit_memo` FOR EACH ROW BEGIN
	...
	IF(OLD.ID <> NEW.ID OR OLD.VCM_TOTAL <> NEW.VCM_TOTAL OR OLD.AP_ACC_ID <> NEW.AP_ACC_ID OR OLD.VCM_ENTRY_DATE <> NEW.VCM_ENTRY_DATE OR OLD.PAYPAL_TRANSACTION_ID <> NEW.PAYPAL_TRANSACTION_ID OR OLD.COMPANY_ID <> NEW.COMPANY_ID)
		then 
...
CREATE TRIGGER `TR_VENDOR_CREDIT_MEMO_DETAIL_UPT` AFTER UPDATE ON `vendor_credit_memo_detail` FOR EACH ROW BEGIN
	...
	IF(OLD.ID <> NEW.ID OR OLD.VENDOR_CREDIT_MEMO_ID <> NEW.VENDOR_CREDIT_MEMO_ID OR OLD.AMOUNT <> NEW.AMOUNT OR OLD.GL_ACC_ID <> NEW.GL_ACC_ID OR OLD.ENTRY_DATE <> NEW.ENTRY_DATE)
		then 
...
CREATE TRIGGER `TR_SALE_INVOICE_UPT` AFTER UPDATE ON `sale_invoice` FOR EACH ROW BEGIN 
	...
	if (
		OLD.ID <> NEW.ID OR OLD.SI_TOTAL <> NEW.SI_TOTAL OR 
		OLD.DISCOUNT <> NEW.DISCOUNT OR OLD.AR_ACC_ID <> NEW.AR_ACC_ID OR 
		OLD.SI_ENTRY_DATE <> NEW.SI_ENTRY_DATE OR OLD.PAYPAL_TRANSACTION_ID <> NEW.PAYPAL_TRANSACTION_ID OR OLD.COMPANY_ID <> NEW.COMPANY_ID OR 
...
CREATE TRIGGER `TR_SALE_INVOICE_DETAIL_UPT` AFTER UPDATE ON `sale_invoice_detail` FOR EACH ROW BEGIN
	...
	if(
		OLD.ID <> NEW.ID OR OLD.SALE_INVOICE_ID <> NEW.SALE_INVOICE_ID OR 
...
CREATE TRIGGER `TR_SALE_RETURN_UPT` AFTER UPDATE ON `sale_return` FOR EACH ROW BEGIN 
	...
	if(
		OLD.ID <> NEW.ID OR OLD.SR_TOTAL <> NEW.SR_TOTAL OR 
		OLD.AR_ACC_ID <> NEW.AR_ACC_ID OR OLD.SR_ENTRY_DATE <> NEW.SR_ENTRY_DATE OR 
		OLD.PAYPAL_TRANSACTION_ID <> NEW.PAYPAL_TRANSACTION_ID OR OLD.COMPANY_ID <> NEW.COMPANY_ID OR 
		OLD.SALES_TAX <> NEW.SALES_TAX OR OLD.TAX_ACC_ID <> NEW.TAX_ACC_ID OR OLD.FREIGHT_CHARGES <> NEW.FREIGHT_CHARGES OR 
...
CREATE TRIGGER `TR_SALE_RETURN_DETAIL_UPT` AFTER UPDATE ON `sale_return_detail` FOR EACH ROW BEGIN
	...
	IF(
		OLD.SALE_RETURN_ID <> NEW.SALE_RETURN_ID OR OLD.ID <> NEW.ID OR OLD.GL_ACC_ID <> NEW.GL_ACC_ID OR
...
CREATE TRIGGER `TR_REPLACEMENT_UPT` AFTER UPDATE ON `replacement` FOR EACH ROW BEGIN
	...
	if(
		OLD.ID <> NEW.ID OR OLD.AMOUNT_RETURN <> NEW.AMOUNT_RETURN OR OLD.SALES_TAX <> NEW.SALES_TAX OR 
		OLD.AR_ACC_ID <> NEW.AR_ACC_ID OR OLD.REP_ENTRY_DATE <> NEW.REP_ENTRY_DATE OR OLD.PAYPAL_TRANSACTION_ID <> NEW.PAYPAL_TRANSACTION_ID OR 
		OLD.COMPANY_ID <> NEW.COMPANY_ID OR OLD.AMOUNT_ISSUE <> NEW.AMOUNT_ISSUE OR OLD.MISCELLANEOUS_CHARGES <> NEW.MISCELLANEOUS_CHARGES OR 
...
CREATE TRIGGER `TR_REPLACEMENT_DETAIL_UPT` AFTER UPDATE ON `replacement_detail` FOR EACH ROW BEGIN
	...
	if(
		OLD.ID <> NEW.ID OR OLD.IS_RETURN<> NEW.IS_RETURN OR OLD.REPLACEMENT_ID <> NEW.REPLACEMENT_ID OR 
		OLD.AMOUNT <> NEW.AMOUNT OR OLD.GL_ACC_ID <> NEW.GL_ACC_ID OR OLD.COS_ACC_ID <> NEW.COS_ACC_ID OR 
...
CREATE TRIGGER `TR_STOCK_TRANSFER_UPT` AFTER UPDATE ON `stock_transfer` FOR EACH ROW BEGIN
	...
	if(
		OLD.ID <> NEW.ID OR OLD.ST_TOTAL <> NEW.ST_TOTAL OR OLD.AR_ACC_ID <> NEW.AR_ACC_ID OR
		OLD.ST_ENTRY_DATE <> NEW.ST_ENTRY_DATE OR OLD.ST_ID <> NEW.ST_ID OR OLD.COMPANY_FROM_ID <> NEW.COMPANY_FROM_ID OR 
		OLD.FREIGHT_CHARGES <> NEW.FREIGHT_CHARGES OR OLD.FREIGHT_ACC_ID <> NEW.FREIGHT_ACC_ID OR 
		OLD.CUSTOM_CHARGES <> NEW.CUSTOM_CHARGES OR OLD.CUS_ACC_ID <> NEW.CUS_ACC_ID
	  )
	then 
...
CREATE TRIGGER `TR_STOCK_TRANSFER_DETAIL_UPT` AFTER UPDATE ON `stock_transfer_detail` FOR EACH ROW BEGIN
	...
	if(
			OLD.ID <> NEW.ID OR OLD.STOCK_TRANSFER_ID <> NEW.STOCK_TRANSFER_ID OR OLD.AMOUNT <> NEW.AMOUNT OR 
...
CREATE  TRIGGER `TR_STOCK_IN_UPT` AFTER UPDATE ON `stock_in` FOR EACH ROW BEGIN
	...
	if(
		OLD.ID <> NEW.ID OR OLD.TOTAL_AMOUNT_IN <> NEW.TOTAL_AMOUNT_IN OR OLD.AP_ACC_ID <> NEW.AP_ACC_ID OR 
		OLD.SN_ENTRY_DATE <> NEW.SN_ENTRY_DATE OR OLD.SN_ID <> NEW.SN_ID OR OLD.COMPANY_ID <> NEW.COMPANY_ID
...
CREATE TRIGGER `TR_STOCK_IN_DETAIL_UPT` AFTER UPDATE ON `stock_in_detail` FOR EACH ROW BEGIN
	...
	if(
			OLD.ID <> NEW.ID OR OLD.STOCK_IN_ID <> NEW.STOCK_IN_ID OR 
...
CREATE TRIGGER `TR_ADJUSTMENT_DETAIL_UPT` AFTER UPDATE ON `adjustment_detail` FOR EACH ROW BEGIN
	...
	if(
		OLD.ID <> NEW.ID OR OLD.IS_QUANTITY_ADDED <> NEW.IS_QUANTITY_ADDED OR OLD.OLD_ESN <> NEW.OLD_ESN OR 
		OLD.ADJUSTMENT_ID <> NEW.ADJUSTMENT_ID OR OLD.NEW_AMOUNT <> NEW.NEW_AMOUNT OR OLD.GL_ACC_ID <> NEW.GL_ACC_ID OR 
...
CREATE TRIGGER `TR_REPAIR_OUT_DETAIL_UP` AFTER UPDATE ON `repair_out_detail` FOR EACH ROW BEGIN
	...
	if(
		OLD.ID <> NEW.ID OR OLD.REPAIR_OUT_ID <> NEW.REPAIR_OUT_ID OR OLD.AMOUNT <> NEW.AMOUNT OR OLD.INV_ACC_ID <> NEW.INV_ACC_ID OR 
